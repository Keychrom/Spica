...
flowchart TD
    subgraph Spica_Node ["Spica ノード (自立サーバー)"]
        ...
        Client <-->|"REST API / SSE（リアルタイム配信）"| Server
        Server <--> DB
        Client <--> WebAuthn
    end
...
| **高度なプライバシー設定** | 公開、ローカル限定、フォロワー限定の3段階の公開範囲制御に加え、強力なミュート・ブロック機能。DM（1対1のメッセージ）は、運営者に電気通信事業法上の義務が及ぶおそれを避けるため、方針として実装していません（詳細は [README](../README.md) の「DM を実装しない方針」を参照）。 |
