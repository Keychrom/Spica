# 🛠️ 設置ガイド（通常構成） — SQLite・1 プロセス

> ⚠️ **現在は Alpha 版の開発中ソフトウェアです。実運用（本番環境での利用）は想定されていません。**
>
> ⚠️ **本ドキュメントに記載されている手順は、理論上可能な手順を書いているだけであり、実際に導入可能かどうかなどは一切検証していません。**

**この構成は「少人数（〜数十人）」向けです。** SQLite 1 ファイルと Node.js だけで動き、追加のミドルウェアは要りません。
Ubuntu などの Linux サーバーで、インターネットに公開するところまでをこの 1 枚で説明します。

人数が増えてきたら **[PostgreSQL + Redis 構成 (SETUP_PostgreSQL_Redis.md)](SETUP_PostgreSQL_Redis.md)** へ移れます。
なぜ向き不向きがあるのかは **[規模と安定性について (SCALE.md)](SCALE.md)** にまとめています。

| | この構成（通常） | PostgreSQL + Redis 構成 |
| :--- | :--- | :--- |
| データベース | SQLite（ファイル 1 つ） | PostgreSQL |
| 追加で必要なもの | なし（Node.js だけ） | PostgreSQL と Redis |
| プロセス | 1 つ | 複数（ロードバランサで振り分け） |
| 向いている規模 | 〜数十人 | 数百人〜（**検証はこれから**） |
| 手順 | この文書 | [SETUP_PostgreSQL_Redis.md](SETUP_PostgreSQL_Redis.md) |

> [!TIP]
> **迷ったらこの構成で始めてください。** あとから PostgreSQL + Redis へ移れます（データの移送手順も用意しています）。
...
- 設定できる項目の一覧は **[設定リファレンス (CONFIGURATION.md)](CONFIGURATION.md)** にあります。
- `DOMAIN` は**必ず実際に外部からアクセスされるホスト名と完全一致**させてください（連合の署名検証に使います）。
...
server {
    ...
    location / {
        ...
        # SSE をバッファリングしない（これが無いとリアルタイム更新が遅延します）
        proxy_buffering off;
...
> [!NOTE]
> 上のコマンドは**1 プロセス**で動かします（この構成の前提）。同じノードを複数のプロセスで動かす場合は、
> **PostgreSQL と Redis が前提**になります（メディアも R2 / S3 へ）。手順は
> **[PostgreSQL + Redis 構成 (SETUP_PostgreSQL_Redis.md)](SETUP_PostgreSQL_Redis.md)** にまとめています。
...
## 🚀 ステップ 5: メディアオブジェクトストレージ (Cloudflare R2・任意)

投稿画像や添付ファイルを外部ストレージに保存したい場合、Cloudflare R2（無料枠：毎月10GB・転送量無料）が最適です。
**1 プロセスで動かす分にはローカルディスク（`server/data/uploads`）のままで構いません**が、次の場合は必須になります。

- 複数のホストで動かす（ローカルディスクは共有されません）
- サーバーのディスク容量を節約したい
...
この構成が向いているのは**同時に数人が使う規模**までです。次の兆候が出たら、次の段階を検討してください。

- 書き込み（投稿・リアクション・配送）が混み合って待たされる
- DB ファイルの増加が速すぎて、掃除が追いつかない
- CPU を使い切っているのに、待ち時間が減らない（1 プロセスの限界）

移行先は **[PostgreSQL + Redis 構成 (SETUP_PostgreSQL_Redis.md)](SETUP_PostgreSQL_Redis.md)** です。
順番の目安と「どのくらいから厳しくなるのか」は **[規模と安定性について (SCALE.md)](SCALE.md)** にまとめています。

| やること | 効果 | 手順 |
| :--- | :--- | :--- |
| リレーの購読を見直す | いちばん効きます（流入とディスクの増え方が変わる） | 管理画面 |
| 保持期間・件数上限を決める | DB の増加 | [CONFIGURATION.md](CONFIGURATION.md) |
| メディアを R2 / S3 へ | ディスクと帯域 | 上のステップ 5 |
| PostgreSQL へ移す | 書き込みの詰まり | [POSTGRESQL.md](POSTGRESQL.md) / [SETUP_PostgreSQL_Redis.md](SETUP_PostgreSQL_Redis.md) |
| Redis を入れて複数プロセスに | CPU の頭打ちとリアルタイム更新 | [REDIS.md](REDIS.md) |
