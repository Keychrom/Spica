...
- **ドメインブロック**: 悪質サーバーやスパムの遮断（該当ドメインからの投稿・配送・フォローを拒否）
...
サーバー常駐のまま、毎日 1 回だけ「バックアップ → 保存・索引の方針の適用 → 保持期間を超えたリモート投稿の削除」を行います。空いた領域を実際に解放する **VACUUM は自動では行いません**（排他ロックが必要なため、サーバーを停止して `npm run db:maintenance -- --apply` を実行します）。
