...
- サーバールール・利用規約 URL・プライバシーポリシー URL・連絡先・運営者・リポジトリ URL
  （**連絡先**を設定すると `/.well-known/security.txt`（RFC 9116）を配信します。未設定なら 404 です）
...
| `RECENT_SCAN_POSTS` | `20000` | タグタイムラインと検索が走査する「直近の投稿数」。本文の部分一致（`LIKE`）は索引が使えないため、直近 N 件に限って全件走査を避けます（実測: 2 万件 17ms / 5 万件 40ms / 10 万件 139ms）。広げるほど古い投稿のタグ・語も見つかります。`0` で無制限（従来の挙動）。 |

### メディアの CDN（任意）

| 環境変数名 | デフォルト | 説明 |
| :--- | :--- | :--- |
| `MEDIA_PUBLIC_BASE_URL` | （空） | 設定すると、API の応答に含まれる自分のメディア（`/uploads/...`）をこの URL に置き換えます（DB は相対のままなので後から替えられます）。前段に Cloudflare などの CDN を置き、`/uploads` をオリジンとして取り込ませてください。 |

アップロードのファイル名は `<時刻>_<乱数>.<拡張子>` で、中身が変われば名前も変わります。
そのため `/uploads` は `Cache-Control: public, max-age=31536000, immutable` で配信しており、
CDN 側でも長くキャッシュできます（差し替えは新しい URL になります）。

### メトリクス（`/metrics`・任意）

| 環境変数名 | デフォルト | 説明 |
| :--- | :--- | :--- |
| `METRICS_TOKEN` | （空） | 設定すると `/metrics`（Prometheus 形式）が有効になります。**未設定なら 404** です。 |

Prometheus は `Authorization: Bearer <トークン>` で取得します（`bearer_token` 設定）。
...
出す値: リクエスト数（状態コードの分類ごと）・応答時間の合計と件数・SSE の接続数・配送とジョブの滞留・
タイムラインキャッシュのヒット数・DB と Redis の状態・RSS と**開いているハンドル数**（増え続けたら
リークの目印）。管理者のセッションでも読めます（`/health` の詳細と同じ扱いです）。

> [!NOTE]
> 接続元 IP では守っていません（nginx の背後では全部 127.0.0.1 になるため）。**トークンで守ってください。**
