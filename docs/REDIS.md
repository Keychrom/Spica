...
| **リアルタイム更新（SSE）** | 自分のプロセスに接続した人にしか届かない | Pub/Sub で全プロセスに届く（どのプロセスに繋いでいても同じ） |
...
- **配送の並列度は上がりません。** ロックで「1 プロセスだけが実行する」ようにしているので、
  二重送信は防げますが、速くはなりません。並列化するなら `outbox_deliveries` を
  `SELECT ... FOR UPDATE SKIP LOCKED` で取り出す形に変える必要があります（未実装）。
- **ジョブキューではありません。** 予約投稿・配送再送・メンテナンスは、いまもアプリ内のタイマーです。
  Redis を入れると「1 プロセスだけが実行する」ようになるだけです。
- **タイムラインのキャッシュには使っていません。** いま Redis に置いているのは
  カウンタ・Pub/Sub・ロックだけです（キャッシュ層は未実装）。
