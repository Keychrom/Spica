...
export async function deleteUserAccount(userId: string): Promise<DeleteUserAccountResult> {
  ...
  const user = await adb.prepare('SELECT * FROM users WHERE id = ?').get(cleanId) as UserRow | undefined;
  if (!user) {
  ...
  try {
    const userPosts = await adb.prepare('SELECT id, media_attachments FROM posts WHERE user_id = ?').all(cleanId) as {
      id: string;
  ...
  try {
    ...
    await adb.prepare('DELETE FROM reactions WHERE post_id IN (SELECT id FROM posts WHERE user_id = ?)').run(cleanId);
    await adb.prepare('DELETE FROM announces WHERE post_id IN (SELECT id FROM posts WHERE user_id = ?)').run(cleanId);
    await adb.prepare('DELETE FROM bookmarks WHERE post_id IN (SELECT id FROM posts WHERE user_id = ?)').run(cleanId);
    await adb.prepare('DELETE FROM pinned_posts WHERE post_id IN (SELECT id FROM posts WHERE user_id = ?)').run(cleanId);
    await adb.prepare('DELETE FROM polls WHERE post_id IN (SELECT id FROM posts WHERE user_id = ?)').run(cleanId);

    // ユーザー自身が行ったアクション
    await adb.prepare('DELETE FROM reactions WHERE user_id = ?').run(cleanId);
    await adb.prepare('DELETE FROM announces WHERE user_id = ?').run(cleanId);
    await adb.prepare('DELETE FROM bookmarks WHERE user_id = ?').run(cleanId);
    await adb.prepare('DELETE FROM pinned_posts WHERE user_id = ?').run(cleanId);
    await adb.prepare('DELETE FROM poll_votes WHERE user_id = ?').run(cleanId);
    await adb.prepare('DELETE FROM user_blocks WHERE user_id = ? OR target_user_id = ?').run(cleanId, cleanId);
    await adb.prepare('DELETE FROM user_mutes WHERE user_id = ? OR target_user_id = ?').run(cleanId, cleanId);

    // フォロー・フォロワー
    await adb.prepare('DELETE FROM follows WHERE follower_url = ? OR following_url = ?').run(actorUrl, actorUrl);

    // 通知
    await adb.prepare('DELETE FROM notifications WHERE user_id = ? OR actor_id = ?').run(cleanId, actorUrl);

    // 投稿本体
    await adb.prepare('DELETE FROM posts WHERE user_id = ?').run(cleanId);
    ...
    // セッション
    await adb.prepare('DELETE FROM sessions WHERE user_id = ?').run(cleanId);

    // 招待コードのメモ更新（管理者が作成した招待コードの参照を保護）
    await adb.prepare("UPDATE invitation_codes SET memo = memo || ' (作成者退会)' WHERE created_by = ?").run(cleanId);

    // ユーザー本体
    await adb.prepare('DELETE FROM users WHERE id = ?').run(cleanId);
