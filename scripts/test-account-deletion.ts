...
async function runTest() {
  ...
  const aliceUser = db.prepare('SELECT * FROM users WHERE id = ?').get('alice');
  if (aliceUser) throw new Error('alice の users レコードが残っています');

  const alicePosts = db.prepare('SELECT * FROM posts WHERE user_id = ?').all('alice');
  if (alicePosts.length > 0) throw new Error('alice の posts レコードが残っています');

  const alicePostReactions = db.prepare("SELECT * FROM reactions WHERE post_id IN ('post_alice_1', 'post_alice_2')").all();
  if (alicePostReactions.length > 0) throw new Error('alice の投稿に対するリアクションが残っています');

  const aliceReactions = db.prepare('SELECT * FROM reactions WHERE user_id = ?').all('alice');
  if (aliceReactions.length > 0) throw new Error('alice が付けたリアクションが残っています');

  const bobReactions = db.prepare('SELECT * FROM reactions WHERE user_id = ?').all('bob');
  if (bobReactions.length > 0) throw new Error('aliceの投稿につけたbobのリアクションが残っています');

  const aliceFollows = db.prepare("SELECT * FROM follows WHERE follower_url LIKE '%/alice' OR following_url LIKE '%/alice'").all();
  if (aliceFollows.length > 0) throw new Error('alice のフォロー関係が残っています');

  const aliceSessions = db.prepare('SELECT * FROM sessions WHERE user_id = ?').all('alice');
  if (aliceSessions.length > 0) throw new Error('alice のセッションが残っています');

  // bobの投稿が残っていることの確認
  const bobPost = db.prepare("SELECT * FROM posts WHERE id = 'post_bob_1'").get();
