...
- 🔔 **PWA ＆ Web Push 通知**
  ...
  - **通知の種類別設定**で「リアクションだけ切る」「フォロー通知だけ残す」といった調整ができます（切った種類はアプリ内・プッシュの両方に届きません）。
- 🗂️ **ドライブ（自分のアップロード管理）**
  - アップロードした画像・動画・音声を一覧でき、使用量（件数・サイズ）と投稿での使用状況を確認できます。
  - 投稿に使っていないファイルはドライブから削除できます（使用中のファイルは投稿を消すまで保護されます）。`MEDIA_QUOTA_MB` で容量上限も設定できます。
- 🎞️ **動画サムネイル（ffmpeg 連携・任意）**
  - ffmpeg が入っていれば、動画の先頭付近からサムネイルを自動生成し、タイムラインでは静止画＋再生ボタンで表示します（動画の先読みが減り軽くなります）。
  - 未インストールでもアップロードは成功し、サムネイルが付かないだけです。
...
- 🔒 **3段階の公開範囲**
  - 公開（連合配信）／ローカル限定／**フォロワー限定**を投稿ごとに選択できます。フォロワー限定の投稿は承認済みフォロワーのみが閲覧でき、連合先へはフォロワーの Inbox にだけ届きます（リレーには配信されません）。**DM（1対1のメッセージ）は方針として実装していません**（理由は「DM を実装しない方針」の節を参照）。
- 🚩 **通報とモデレーション**
